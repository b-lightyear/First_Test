...
<body>
	...
	<div class="container-fluid px-4">
		...
		<table class='table'>
			...
			<tr>
				<th>첨부파일</th>
				<td>${dto.filename}<c:if test='${dto.filename != null}'>
						<a href='download.no?id=${dto.id}'>
		...
		<div class='btnSet'>
			<a class='btn-fill' href='list.no'>목록</a>
			
			<!-- 관리자회원만 가능한 기능 -->
			<%-- <c:if test='${userInfo.admin eq "Y"}'>  --%>
			
			<!-- 작성자로 로그인 한 경우만 수정/삭제 가능 -->
			<c:if test='${userInfo.userid eq dto.writer}'>
			<a class='btn-fill' href='modify.no?id=${dto.id}'>수정</a>
			<a class='btn-fill' href='delete.no?id=${dto.id}'>삭제</a>
			</c:if>
			
			<!-- 로드인 된 경우 답글쓰기 가능 -->
			<c:if test ='${!empty userInfo }'>
			<a class='btn-fill' href='reply.no?id=${dto.id}'>답글 쓰기</a>
			</c:if>
		</div>
	</div>
